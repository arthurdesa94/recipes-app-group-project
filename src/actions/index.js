...
export const retrievefirstLetterRecipes = (value) => (dispatch) => {
  dispatch({ type: LOADING });
  API.searchFoodFirstWordDrinkRequest(value).then((result) =>
...
export const retrieveIngredientRecipes = (value) => (dispatch) => {
  dispatch({ type: LOADING });
  API.searchFoodByMainIngredientsRequest(value).then((result) =>
    dispatch(storageRecipes(result)),
  );
  dispatch({ type: OK });
};
...
export const retrieveDrinkNameRecipes = (value) => (dispatch) => {
  dispatch({ type: LOADING });
  DRINKAPI.searchDrinkNameRequest(value).then((result) => 
  dispatch(storageDrinkRecipes(result)),
  );
  dispatch({ type: OK });
};
...
export const retrieveDrinkFirstLetterRecipes = (value) => (dispatch) => {
  dispatch({ type: LOADING });
  DRINKAPI.searchDrinkFirstLetterRequest(value).then((result) =>
    dispatch(storageDrinkRecipes(result)),
  );
  dispatch({ type: OK });
};
...
export const retrieveDrinkIngredientRecipes = (value) => (dispatch) => {
  dispatch({ type: LOADING });
  DRINKAPI.searchDrinkByMainIngredientsRequest(value).then((result) =>
    dispatch(storageDrinkRecipes(result)),
  );
  dispatch({ type: OK });
};
